...
def _series_delta(series: pd.Series, lag_days: int) -> float | None:
...
def render_overview(df_tx, df_trade_priced, df_prices_long, df_master) -> None:
    ...
    delta_port_1d = _series_delta(chart["portfolio_value"], 1)
    delta_cash_1d = _series_delta(chart["cash_balance"], 1)
    delta_net_30d = _series_delta(chart["net_invested"], 30)
    delta_pnl_1d = _series_delta(pnl_series, 1)
    delta_port_30d = _series_delta(chart["portfolio_value"], 30)
    delta_pnl_30d = _series_delta(pnl_series, 30)
