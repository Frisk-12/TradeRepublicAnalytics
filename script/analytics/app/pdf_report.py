...
def _series_delta(series: pd.Series, lag: int, *, precision: int | None = None) -> float | None:
...
def generate_portfolio_pdf_report(df_tx, df_trade_priced, df_prices_long, df_master) -> bytes:
    ...
    delta_port_1d = _series_delta(chart["portfolio_value"], 1, precision=2) if "portfolio_value" in chart.columns else None
    delta_cash_1d = _series_delta(chart["cash_balance"], 1, precision=2) if "cash_balance" in chart.columns else None
    delta_net_30d = _series_delta(chart["net_invested"], 30, precision=2) if "net_invested" in chart.columns else None
    delta_pnl_1d = _series_delta(pnl_series, 1, precision=2) if not pnl_series.empty else None
