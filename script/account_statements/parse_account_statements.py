...
AMOUNT_PREFIX_RE = re.compile(
    r"(?<![\d.,])(?:€|EUR)\s{0,1}(?P<num>-?(?:\d{1,3}(?:[.,\s\u00A0\u202F]\d{3})+|\d+)(?:[.,]\d{2}))\b(?!\s*(?:€|EUR))",
    flags=re.IGNORECASE,
)
...
def _parse_amounts(line: str) -> List[float]:
    # Ignore trade quantity fragments so they cannot be misread as monetary amounts.
    line_clean = re.sub(
        r"\b(quantity|quantite|quantité|quantita)\s*:\s*[0-9]+(?:[.,][0-9]+)?\b",
        "",
        line,
        flags=re.IGNORECASE,
    )
    found: List[Tuple[int, str]] = []
    for m in AMOUNT_SUFFIX_RE.finditer(line_clean):
        found.append((m.start(), m.group("num")))
    for m in AMOUNT_PREFIX_RE.finditer(line_clean):
        found.append((m.start(), m.group("num")))
