...
def _fallback_name_by_isin(data: Dict[str, Dict]) -> Dict[str, Optional[str]]:
    ...
    for payload in data.values():
        for txn in payload.get("transactions", []):
            ...
            if not isin:
                continue
            name = (trade.get("instrument_name") or "").strip()
            if name and isin not in out:
                out[isin] = name
    return out
...
def enrich_account_statements(
    ...
    fallback_names = _fallback_name_by_isin(data)
    ...
    if missing and api_key:
        ...
        openfigi_mode = "cache_only_no_api_key"
        openfigi_warning = (
            "OPENFIGI_API_KEY not configured: running with cache-only / fallback instrument metadata."
        )
    elif not api_key:
        openfigi_mode = "cache_only_no_api_key"
        openfigi_warning = (
            "OPENFIGI_API_KEY not configured: running with cached instrument metadata."
        )

    security_master: Dict[str, dict] = {}
    for isin in isins:
        base = cache.get(isin, {"isin": isin})
        if isin not in cache and not api_key:
            base = {
                "isin": isin,
                "name": fallback_names.get(isin),
                "status": "no_api_key",
                "candidates": 0,
            }
    ...
    return {
        ...
        "statements": data,
        "openfigi_report": {
            **_quality_report(security_master),
            "mode": openfigi_mode,
            "warning": openfigi_warning,
