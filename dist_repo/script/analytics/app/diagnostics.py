...
def render_diagnostics(df_master: pd.DataFrame, df_tx: pd.DataFrame, df_trade_priced: pd.DataFrame) -> None:
    ...
    st.markdown("**Fiscal — FIFO & Zainetto**")
    if "minus_pool_eur" not in d.columns:
        st.info("Dati fiscali non disponibili per questa versione.")
    else:
        ...
        with st.expander("Dettaglio fiscale (FIFO / Zainetto)", expanded=False):
            st.dataframe(
                fiscal.style.format(
                    {
                        "price_used": "{:.4f}",
    ...
    isin_text = st.text_input("Filtro ISIN contiene", "", key="diag_isin_filter")
    if isin_text:
