...
def render_performance(df_tx, df_trade_priced, df_prices_long, df_master) -> None:
    ...
    if account_series_ex_pm.empty:
        st.info("Dati insufficienti per la performance.")
        return
    ...
    c1.metric(
        "Total Return (TWR)",
        _pct(total_return),
        help="TWR chain‑linked: prodotto dei rendimenti giornalieri flow‑adjusted meno 1 (flussi esterni = Bonifici).",
    )
    c2.metric(
        "Annualized Return",
        _pct(ann_return) if not np.isnan(ann_return) else "n/a",
        help="Annualizzazione del TWR totale; mostrata solo se il periodo è sufficiente.",
    )
    c3.metric(
        "Volatility",
        _pct(vol),
        help="Deviazione standard dei rendimenti giornalieri TWR, annualizzata con √252.",
    )
    c4.metric(
        "Sharpe Ratio",
        f"{sharpe:.2f}" if not np.isnan(sharpe) else "n/a",
        help="Sharpe (rf=0): rendimento medio annuo / volatilità annua.",
    )
    ...
    with c_left:
        ...
        st.dataframe(_table_style(styler), use_container_width=True, hide_index=True, height=260)
    with c_right:
    ...
    if not price_wide.empty:
        ...
        if items:
            ...
            st.caption("Nessuna posizione con estremi rilevanti nel periodo recente.")
