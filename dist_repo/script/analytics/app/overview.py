...
def render_overview(df_tx, df_trade_priced, df_prices_long, df_master) -> None:
    ...
    c1.metric(
        ...
        help="Valore di mercato attuale delle posizioni aperte: somma(qty_net ISIN x ultimo prezzo disponibile).",
    )
    c2.metric(
        "Cash Balance",
        _eur(res.cash_balance),
        help="Ultimo saldo disponibile dalle transazioni (colonna saldo).",
    )
    c3.metric(
        "Net Cash In",
        _eur(res.net_cash_in),
        help=(
            "Somma netta dei Bonifici dall'inizio storico a oggi. "
            "Include anche eventuali prelievi se registrati come Bonifico negativo."
        ),
    )
    c4.metric(
        "PnL",
        _eur(res.pnl_abs),
        help=(
            "Profit/Loss assoluto: (Account Value - Private Markets) - Net Invested. "
            "I Private Markets restano a valore nominale finché non realizzati."
        ),
    )
    ...
    if df_trade_priced.empty or df_prices_long.empty:
        st.info("Dati insufficienti per calcolare i contributors.")
    else:
        ...
        with st.expander("All Positions — PnL (weight ≥ 0.5%)"):
            ...
            st.caption("Mostra solo posizioni con peso ≥ 0.5% del portfolio value.")
    ...
    with st.expander("Dettaglio movimenti Account Value (dataframe)"):
    ...
    if df_trade_priced.empty or df_prices_long.empty or df_master is None:
        st.info("Dati insufficienti per calcolare le allocazioni.")
        return
    ...
    country_alloc = (
        pd.DataFrame(country_rows_etf)
        .groupby("country", as_index=False)["market_value"]
        .sum()
        .sort_values("market_value", ascending=False)
    )
    ...
    sector_alloc = (
        pd.DataFrame(sector_rows_etf)
        .groupby("sector", as_index=False)["market_value"]
        .sum()
        .sort_values("market_value", ascending=False)
    )
    ...
    with tab_macro:
        ...
        st.caption("Dettaglio per asset class.")
        ...
        with st.expander("Equity — Details"):
            ...
            if eq_detail.empty:
                st.caption("Nessuna posizione equity.")
            else:
                ...
                st.dataframe(eq_styler, use_container_width=True, hide_index=True)

        with st.expander("ETF — Details"):
            etf_detail = etf_base.copy()
            if etf_detail.empty:
                st.caption("Nessuna posizione ETF.")
            else:
        ...
        with st.expander("Warrant / Derivatives — Details"):
            ...
            if w_base.empty:
                st.caption("Nessun warrant/derivative in portafoglio.")
            else:
    ...
    with tab_equity:
        ...
        st.plotly_chart(fig_country_eq, use_container_width=True)
        ...
        st.plotly_chart(fig_sector_eq, use_container_width=True)

    with st.expander("Allocations — Data"):
